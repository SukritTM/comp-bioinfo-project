...
parser.add_argument('-o', '--output_dir', default='data\\euk_retrieval\\structures\\traintest\\', help='Output directory')
...
for i, uniprot_id in enumerate(tqdm(train['Entry'])):
    ...
    try:
        ...
        adb.download_cif_for(canonical_file, os.path.join(args.output_dir, 'train'))
    except:
        pred = []
...
for i, uniprot_id in enumerate(tqdm(test['Entry'])):
    ...
    try:
        ...
        adb.download_cif_for(canonical_file, os.path.join(args.output_dir, 'test'))
    except:
        pred = []
        no_results.append(uniprot_id)
    # print(i, len(pred))
